...
(function(root, factory) {
  ...
  P.firstChoice = function() {
    ...
    return function(input) {
      var index, length, parser, result;
      index = -1;
      length = parsers.length;
      while (++index < length) {
        parser = parsers[index];
        if ('function' !== typeof parser) {
          throw new Error("parser passed at index `" + index + "` into `firstChoice` is not of type `function` but of type `" + (typeof parser) + "`");
        }
        result = parser(input);
        if (result != null) {
          return result;
  ...
  P.longestChoice = function(parsers) {
    return function(input) {
      var bestResult, index, length, result;
      index = -1;
      length = parsers.length;
      bestResult = null;
      while (++index < length) {
  ...
  P.sequence = function() {
    ...
    parsers = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    return function(input) {
      var i, length, rest, result, values;
      i = -1;
      length = parsers.length;
      values = [];
      rest = input;
